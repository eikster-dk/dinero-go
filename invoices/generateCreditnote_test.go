...
func TestGenerateCreditnoteBasedOnInvoice_integration(t *testing.T) {
	...
	booked, err := Book(c, resp.ID, resp.Timestamp)
	if err != nil {
		t.Error("Booking invoice failed, err: ", err)
	}

	creditnoteParams := GenerateCreditnoteParams{
		InvoiceID: booked.ID,
		Timestamp: booked.Timestamp,
	}

	if _, err := GenerateCreditNote(c, creditnoteParams); err != nil {
		t.Error("generating credit note based on invoice failed: ", err)
	}
}
